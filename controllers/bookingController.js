...
export const createBookingDB = async (req, res, next) => {
  try {
    ...
    if (generatedSignature === razorpay_signature) {
      ...
      res.redirect(`https://react-node-build.onrender.com/payments?status=${true}&order=${orderId}`);
    } else {
      const {
      ...
      res.redirect(
        `https://react-node-build.onrender.com/payments?status=${false}&code=${code}&err=${description}`
      );
    }
  } catch (err) {
